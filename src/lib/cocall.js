/**
 * Created by xinailiu on 2020-01-17 17:45.
 */
/*
          ...
          if ('string' === typeof f) {
            ...
            return !s(p, j)
              ? w(
                  C(
                    'notloaded',
                    'Module name "' + j + '" has not been loaded yet for context: ' + b + (a ? '' : '. Use require([])')
                  )
                )
              : p[j]
          }
...
/**	全局 项目路径*/
var app = { require: require, happurl: 'http://happ.thunisoft.com/happ/resources/js/libs' }
;(function() {
  app.ctx = ''
  app.location = window.location
  app.curWwwPath = ''
  with (app) {
    app.curWwwPath = location.href
    var pathName = location.pathname
    var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1)
    var originName = location.origin
    if (originName) {
      if (projectName.indexOf('/', 0) >= 0) {
        ctx = originName + projectName
      } else {
        ctx = originName + '/' + projectName
      }
    } else {
      var pos = app.curWwwPath.indexOf(pathName)
      var localhostPath = app.curWwwPath.substring(0, pos)
      ctx = localhostPath + projectName
    }
  }
})()
/**	配置requireJs全局 js*/
require.config({
  shim: {
    bootstrap: ['jquery'],
    base64: ['jquery'],
    happjs: ['happtype', 'happprototype'],
    'happ-mobile': ['happtype', 'app'],
    'ui-api': ['happ-mobile', 'logger'],
    'location-api': ['happ-mobile', 'logger'],
    'share-api': ['happ-mobile', 'logger'],
    'speech-api': ['happ-mobile', 'logger']
  },
  paths: {
    jquery: [app.happurl + '/jquery/jquery.min-v1.12.4'],
    base64: [app.happurl + '/jquery/jquery-base64'],
    bootstrap: [app.happurl + '/bootstrap/bootstrap.min-v3.3.5'],
    touch: [app.happurl + '/gesture/mTouch-v0.0.1'],
    drag: [app.happurl + '/gesture/mDrag-v0.0.1'],
    dragloader: [app.happurl + '/gesture/dragloader-v0.0.1'],
    'idcard-util': [app.happurl + '/util/idcardUtil'],
    happjs: [app.happurl + '/happ/happ-js-v2.0.0'],
    happprototype: [app.happurl + '/happ/happ-prototype-v2.0.0'],
    happtype: [app.happurl + '/happ/happ-type-v2.0.0'],
    'ui-api': [app.happurl + '/ui/ui'],
    'location-api': [app.happurl + '/location/location'],
    'share-api': [app.happurl + '/share/share'],
    'speech-api': [app.happurl + '/speech/speech'],
    mobile: [app.happurl + '/mobile/mobile-v2.0.0'],
    'happ-extend': [app.ctx + '/resources/js/libs/extend/happ-extend', app.happurl + '/extend/happ-extend'],
    'happ-extend2': [app.ctx + '/resources/js/libs/extend/happ-extend2', app.happurl + '/extend/happ-extend2'],
    'happ-fixed': [app.ctx + '/resources/js/libs/fixed/happ-fixed', app.happurl + '/fixed/happ-fixed'],
    'happ-fixed2': [app.ctx + '/resources/js/libs/fixed/happ-fixed2', app.happurl + '/fixed/happ-fixed2'],
    app: [app.happurl + '/app/app-config'], //app对象的初始化工作提取
    axios: [app.happurl + '/axios/axios.min'], //axios库
    request: [app.happurl + '/request/request'],
    logger: [app.happurl + '/log/log-v2.0.0']
  }
})
/**
 * 系统相关回调JS
...
;(function() {
  if (!window.happ) {
    ...
    var keyStr
    var version
    if (u.indexOf('HappAndroid')) {
      environment = 'HappAndroid'
      keyStr = 'HappAndroid'
    } else if (u.indexOf('HappIos')) {
      environment = 'HappIos'
      keyStr = 'HappIos'
    } else if (u.indexOf('HappWeChat')) {
      environment = 'HappWeChat'
      keyStr = 'HappWeChat'
    } else {
      environment = 'other'
      name = 'other'
    ...
    if (keyStr) {
      try {
        version = u.substring(u.lastIndexOf(keyStr) + keyStr.length)
      } catch (e) {
        console.log(e)
      }
    }
  ...
  if (happ.agent == 'HappAndroid') {
    require.config({
      paths: {
        'happ-mobile': [app.happurl + '/android/android-v2.0.0']
      }
    })
  } else if (happ.agent == 'HappIos') {
    require.config({
      paths: {
        'happ-mobile': [app.happurl + '/ios/ios-v2.0.0']
      }
    })
  } else {
    //浏览器调试使用
    require.config({
      paths: {
        'happ-mobile': [app.happurl + '/web/web-v2.0.0']
      }
    })
  }
})()
 ...
 */
;(function() {
  //为了老项目移动办案的兼容，同一对象叫android（===mobile）
  require.config({
    shim: {
      //办案办公调用的对象名称不一样
      android: ['location-api', 'share-api', 'speech-api', 'happ-extend', 'happ-extend2', 'happ-fixed', 'happ-fixed2']
    },
    paths: {
      android: [app.happurl + '/mobile/mobile']
    }
  })
})()
